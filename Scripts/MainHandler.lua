-- // Configuration
local REPO_URL = "https://raw.githubusercontent.com/8zj/REPO/TDSHub/" -- [!] REPLACE THIS WITH YOUR GITHUB RAW URL

local function load_script(path)
    local root = "TDS Script/V3/"
    local local_path = root .. path
    
    if isfile and isfile(local_path) then
        return loadstring(readfile(local_path))()
    end
    local url_path = REPO_URL .. path
    
    local success, result = pcall(function()
        return game:HttpGet(url_path)
    end)
    ...
    -- 3. Failure
    warn("[Loader] Failed to load script: " .. path)
    warn("[Loader] Local path checked: " .. local_path)
    warn("[Loader] URL checked: " .. url_path)
    return nil
end

local Logger = load_script("Scripts/Logger.lua")
local PlayerHandler = load_script("Scripts/PlayerHandler.lua")

if not Logger then
    -- Fallback logger if file missing
    Logger = {
        Log = function(t, c) print("[Log]["..(c or "?").."] "..t) end
    }
    warn("Logger module not found, using fallback")
end

if not PlayerHandler then
    warn("PlayerHandler module not found!")
    return
end

local game_state = PlayerHandler.IdentifyGameState()
...
-- // services & main refs
local Services = PlayerHandler.Services
local teleport_service = Services.TeleportService
local marketplace_service = Services.MarketplaceService
local replicated_storage = Services.ReplicatedStorage
local remote_func = Services.RemoteFunction
local remote_event = Services.RemoteEvent
local players_service = Services.Players
local local_player = PlayerHandler.GetLocalPlayer()
...
Logger.Log("Game State: " .. tostring(game_state), "blue")
